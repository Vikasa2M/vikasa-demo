...
The fleet is a ~100-cabinet distributed network (33 per DOT). MARDOT is shaped
...
segment (`cab-i75s-01`…`05`); VELDOT/SABDOT are a metro spread with one I-85
hero each.
...
> **Validated on macOS** (Apple Silicon and Intel). Linux and Windows (WSL2) use
> the same Docker + Go toolchain and should work, but haven't been validated yet.
...
- **No sibling checkouts are required.** The base demo builds without them: the
  pinned `openits-models` packages are committed under `vendor/` (the Go and
  Docker builds both run in vendor mode), and the topology renderer is fetched
  as a pinned release (`github.com/Vikasa2M/vikasa-infra@v0.1.0`) by
  `make topology` — so the base demo needs network access only on the first
  `make demo` (cached afterward), or set `INFRA_GEN` to a local checkout to build
  offline (see `docs/MODELS-PIN.md`). The optional AI segment's
  `make ai-models-pack` reads `../../openits-models-pinned` via `MODELS_DIR` to
  regenerate the YANG pack.
...
### Deployment sizes (small / medium / large)

Pick a footprint by machine capacity with a `SIZE` knob (or the wrapper
targets). Each size is a strict superset of the smaller one, generated from
`fleet.yaml` — nothing is hand-maintained.

| Size | Cabinets | ~Containers | Bring-up | Tour phases |
|---|---|---|---|---|
| `small` (`make demo-small`) | 3 (1/DOT) | ~34 | ~1-2 min | baseline, wan-cut, restore, corridor |
| `medium` (`make demo-medium`) | 18 | ~64 | ~2-3 min | all 6 |
| `large` (`make demo`, default) | 99 (33/DOT) | ~226 | ~6-8 min | all 6 |

`SIZE=small|medium|large` drives both `gen-compose` (which cabinets are
generated) and `LoadFleetInfo` (the tour's cabinet→port map), so **bring up
and tour with the same size** — e.g. `SIZE=medium make demo-tour`. `SCALE=1`
still works as an alias for `small`.

- **small** is each DOT's single I-85 corridor hero (`cab-i85-001` and the
  NC/SC equivalents). The fault (`cab-002`), reversible (`cab-i75s-01`), and
  perception-corridor (`cab-i85-002…010`) cabinets do not exist at this size,
  so those tour phases are unavailable — use `--only baseline,wan-cut,restore,corridor`,
  or run medium/large to record.
- **medium** adds exactly the cabinets the tour drives — MARDOT's full I-85
  perception corridor, its I-75S reversible segment, and the fault cabinet —
  plus each DOT's hero, so all six phases run. It drops only the arterial
  cabinets that give the Fleet Health map its large-network density.
- **large** is the full fleet.

Regenerate with a different size any time by re-running the matching target
(or `SIZE=<size> make gen-compose` then `make demo`).
...
| Perception & Fusion (MARDOT I-85 corridor) | `vikasa-perception-fusion` | http://localhost:3000/d/vikasa-perception-fusion |
...
variable accept `?var-dot=mardot` / `&var-cabinet=cab-002` query params to
...
|---|---|---|---|
| cab-i85-001 | mardot | 18081 | I-85 hero — cut/restore + corridor incident |
| cab-002 | mardot | 18082 | signal-fault target (`expose: true`) |
| cab-i85-101 | veldot | 18083 | I-85 hero |
| cab-i85-201 | sabdot | 18084 | I-85 hero |
...
Useful flags (both modes): `--dot` (default `mardot`; the cabinet-scoped
...
| 6 | reversible | I-75 South Reversible Express Lanes | MARDOT's I-75 South express lanes reverse on a schedule — northbound for the AM peak, southbound for the PM | The direction tile flips through an in-transition sweep; the reversal timeline bands blue/orange; every segment-map marker flips color together |
...
6. Phase 7 prints the ask-the-data preface and every question from
   ...
   mardot` and prints the real count/distinct/diff numbers for Q1's outage
...
- **Cross-domain stream sources not flowing (a tier shows 0 messages).**
  ...
  `curl mardot-regional:8222/leafz`) — confirm the expected leaf remotes are
...
- **A dashboard panel is empty / wrong values.** Two likely causes, in
  ...
  `curl -s http://localhost:8123/ --data "SELECT DISTINCT to_state FROM vikasa_mardot.phase_state_change FORMAT TSV"`.
...
- **Empty Grafana panels right after bring-up.** Grafana lazy-loads panel
  ...
  `curl -s http://localhost:8123/ --data "SELECT count() FROM vikasa_mardot.events_raw WHERE ce_time > now() - INTERVAL 5 MINUTE FORMAT TSV"`.
