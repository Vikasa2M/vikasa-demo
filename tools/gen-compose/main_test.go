...
var dots = []string{"gdot", "ncdot", "scdot"}
...
func TestGenComposeGoldenFullFleet(t *testing.T) {
	...
	if err := run(fleetPath, outDir, false); err != nil {
		t.Fatalf("run: %v", err)
	}
	...
	wantServices := []string{
		"gdot-sink", "ncdot-sink", "scdot-sink",
		"stream-init-gdot", "stream-init-ncdot", "stream-init-scdot",
		"federation-sink",
	}
	for _, c := range cabs {
	...
	// gdot-regional-exporter, ncdot-central-exporter, scdot-dmz-exporter).
	// These used to be hand-written gdot-only entries in the static
	...
	// corridor cabinets, e.g. gdot-cab-i85-001-nats-exporter), alongside the
	...
	for _, want := range []string{"gdot-dmz", "ncdot-dmz", "scdot-dmz", "clickhouse"} {
	...
	for _, svc := range []string{"gdot-sink", "ncdot-sink", "scdot-sink", "federation-sink"} {
		if got := serviceRestart(t, services, svc); got != "on-failure" {
			t.Errorf("%s restart = %q, want %q", svc, got, "on-failure")
		}
	}
	for _, svc := range []string{"stream-init-gdot", "stream-init-ncdot", "stream-init-scdot"} {
	...
	// exactly what a WAN-cut severs). A tier service (e.g. "gdot-central")
	...
	wantSinkEnv := map[string]map[string]string{
		"gdot-sink": {
			"SINK_STREAM":      "VIKASA_GDOT_CENTRAL_D1_D1_0",
			"SINK_CH_DATABASE": "vikasa_gdot",
			"SINK_FILTER":      "vikasa.gdot.>",
		},
		"ncdot-sink": {
			"SINK_STREAM":      "VIKASA_NCDOT_CENTRAL_D1_D1_0",
			"SINK_CH_DATABASE": "vikasa_ncdot",
			"SINK_FILTER":      "vikasa.ncdot.>",
		},
		"scdot-sink": {
			"SINK_STREAM":      "VIKASA_SCDOT_CENTRAL_D1_D1_0",
			"SINK_CH_DATABASE": "vikasa_scdot",
			"SINK_FILTER":      "vikasa.scdot.>",
		},
	}
	...
	// file_sd_configs instead of a hardcoded (gdot-only) target list.
	...
	wantSinkTargets := []string{"gdot-sink:9091", "ncdot-sink:9091", "scdot-sink:9091", "federation-sink:9091"}
...
func TestGenComposeCleansStaleOutputAcrossRuns(t *testing.T) {
	...
	if err := run(fleetPath, outDir, true); err != nil {
	...
	if _, err := os.Stat(filepath.Join(natsDir, "gdot-cab-002.conf")); err == nil {
		t.Error("stale gdot-cab-002.conf from the full-fleet run survived the SCALE=1 rerun")
	}
...
// cabinet): deploy/topology/specs/{gdot,ncdot,scdot}.json are static,
...
// drifting. Originally gdot-only (ncdot/scdot specs didn't exist until Task
