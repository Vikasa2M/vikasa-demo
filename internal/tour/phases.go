...
// checks them.
var allDOTs = []string{"mardot", "veldot", "sabdot"}

// mardotSiblingCabinets are cab-i85-001's two siblings in the mardot database —
// used by the wan-cut phase to prove the cut was targeted, not fleetwide.
var mardotSiblingCabinets = []string{"cab-002", "cab-003"}
...
// cutCabinet is mardot's I-85 corridor cabinet — the one every phase after
...
// faultCabinet is mardot's non-corridor cabinet used for the signal-fault
// phase. Deliberately NOT a corridor cabinet: its fault must be asserted
// against the per-DOT database (vikasa_mardot), not vikasa_federation,
...
All three DOTs -- Marren, Velia, Sabine -- are live: about
...
between the cabinet and MARDOT's regional network. The cabinet doesn't stop
...
the other two DOTs stay completely untouched.`), cabinet, cabinet),
		...
		Assert: func(ctx context.Context) error {
			...
			for _, sibling := range mardotSiblingCabinets {
			...
			// 3. veldot/sabdot are entirely untouched.
			for _, dot := range []string{"veldot", "sabdot"} {
				if err := assertBaseline(ctx, cfg.CH, verify.Database(dot)); err != nil {
...
func buildCorridorPhase(cfg Config, cabinet string, port int, corridorCabinets []string) Phase {
	...
	return Phase{
		Name: "corridor",
		Narration: fmt.Sprintf(strings.TrimSpace(`
Inject a perception incident at %s, MARDOT's I-85 corridor cabinet: camera
...
func buildReversiblePhase(cfg Config) Phase {
	db := verify.Database(cfg.DOT) // mardot
	return Phase{
		Name: "reversible",
		Narration: strings.TrimSpace(`
MARDOT also runs the I-75 South Metro Express Lanes -- barrier-separated
