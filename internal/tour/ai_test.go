...
func TestRunAIPhasesQAOnlyPrintsQuestionsAndGroundTruth(t *testing.T) {
	...
	cfg := AIConfig{DOT: "mardot", PromptsPath: "does/not/exist.md"}
	...
	for i, a := range calls[0] {
		if a == "--dot" && i+1 < len(calls[0]) && calls[0][i+1] == "mardot" {
			found = true
		}
	}
	if !found {
		t.Errorf("verify dedup call %v missing --dot mardot", calls[0])
	}
...
func TestRunAIPhasesQAPrintsRealQuestionsFile(t *testing.T) {
	...
	in := bytes.NewBufferString("\n")
	var out bytes.Buffer
	cfg := AIConfig{DOT: "mardot", PromptsPath: "../../demo/ai/prompts/questions.md"}
