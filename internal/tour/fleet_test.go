...
func TestLoadFleetInfoDerivesSimPortsAndCorridorCabinets(t *testing.T) {
	...
	// cabinet + any Expose-flagged non-hero cabinet, e.g. mardot's cab-002
	...
	want := map[string]int{
		"cab-i85-001": 18081, // mardot hero
		"cab-002":     18082, // mardot fault cabinet (expose:true)
		"cab-i85-101": 18083, // veldot hero
		"cab-i85-201": 18084, // sabdot hero
	}
...
func TestLoadFleetInfoMedium(t *testing.T) {
	t.Setenv("SIZE", "medium")
	info, err := LoadFleetInfo("../../deploy/topology/fleet.yaml")
	if err != nil {
		t.Fatalf("LoadFleetInfo(medium): %v", err)
	}
	// Port-mapped cabinets are corridor||expose, in fleet.yaml order: the 3
	// I-85 heroes plus MARDOT's exposed fault cabinet.
	want := map[string]int{
		"cab-i85-001": 18081,
		"cab-002":     18082,
		"cab-i85-101": 18083,
	...
	if len(info.SimPorts) != len(want) {
		t.Fatalf("medium got %d sim ports, want %d: %+v", len(info.SimPorts), len(want), info.SimPorts)
	}
	for cab, p := range want {
		if got := info.SimPorts[cab]; got != p {
			t.Errorf("medium SimPorts[%q] = %d, want %d", cab, got, p)
		}
	}
}

func TestLoadFleetInfoRejectsBadSize(t *testing.T) {
	t.Setenv("SIZE", "tiny")
	if _, err := LoadFleetInfo("../../deploy/topology/fleet.yaml"); err == nil {
		t.Fatal("LoadFleetInfo with SIZE=tiny: want error, got nil")
	}
}
