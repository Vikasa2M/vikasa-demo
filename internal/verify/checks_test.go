...
func TestDatabase(t *testing.T) {
	if got, want := Database("gdot"), "vikasa_gdot"; got != want {
		t.Errorf("Database(gdot) = %q, want %q", got, want)
	}
	if got, want := Database("ncdot"), "vikasa_ncdot"; got != want {
		t.Errorf("Database(ncdot) = %q, want %q", got, want)
	}
}
...
func TestQueryBuilders(t *testing.T) {
	cases := []struct {
		...
		{
			"RecentEventsQuery",
			RecentEventsQuery("vikasa_gdot"),
			"SELECT count() FROM vikasa_gdot.events_raw WHERE ce_time > now() - INTERVAL 2 MINUTE FORMAT TSV",
		},
		{
			"HeartbeatFreshnessQuery",
			HeartbeatFreshnessQuery("vikasa_gdot"),
			"SELECT dateDiff('second', max(ingested_at), now64(3)) FROM vikasa_gdot.heartbeats FORMAT TSV",
		},
		{
			"DeadLetterQuery",
			DeadLetterQuery("vikasa_gdot"),
			"SELECT count() FROM vikasa_gdot.events_dead_letter WHERE received_at > now() - INTERVAL 1 HOUR FORMAT TSV",
		},
		{
			"OptimizeEventsRawQuery",
			OptimizeEventsRawQuery("vikasa_gdot"),
			"OPTIMIZE TABLE vikasa_gdot.events_raw FINAL",
		},
		{
			"DedupCountsQuery",
			DedupCountsQuery("vikasa_gdot"),
			"SELECT count(), countDistinct(ce_id) FROM vikasa_gdot.events_raw WHERE ce_time > now() - INTERVAL 30 MINUTE FORMAT TSV",
		},
	}
...
func TestCheckRecentEventsPassFail(t *testing.T) {
	...
	r, err := CheckRecentEvents(context.Background(), Deps{CH: srv.URL, DB: "vikasa_gdot"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !r.Pass {
	...
	srv2, _ := stubCH(t, "0\n")
	r2, err := CheckRecentEvents(context.Background(), Deps{CH: srv2.URL, DB: "vikasa_gdot"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if r2.Pass {
...
func TestCheckHeartbeatFreshnessNormalAndExpectStale(t *testing.T) {
	...
	r, err := CheckHeartbeatFreshness(context.Background(), Deps{CH: srvFresh.URL, DB: "vikasa_gdot"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !r.Pass {
	...
	rStale, err := CheckHeartbeatFreshness(context.Background(), Deps{CH: srvFresh.URL, DB: "vikasa_gdot", ExpectStale: true})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if rStale.Pass {
	...
	r2, err := CheckHeartbeatFreshness(context.Background(), Deps{CH: srvStale.URL, DB: "vikasa_gdot"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if r2.Pass {
	...
	r2Stale, err := CheckHeartbeatFreshness(context.Background(), Deps{CH: srvStale.URL, DB: "vikasa_gdot", ExpectStale: true})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !r2Stale.Pass {
...
func TestCheckDeadLetters(t *testing.T) {
	...
	r, err := CheckDeadLetters(context.Background(), Deps{CH: srvClean.URL, DB: "vikasa_gdot"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !r.Pass {
	...
	r2, err := CheckDeadLetters(context.Background(), Deps{CH: srvDirty.URL, DB: "vikasa_gdot"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if r2.Pass {
...
func TestCheckReturnsErrorOnHTTPFailure(t *testing.T) {
	...
	if _, err := CheckRecentEvents(context.Background(), Deps{CH: srv.URL, DB: "vikasa_gdot"}); err == nil {
...
func TestRunBaselineReportsAllChecksInOrder(t *testing.T) {
	...
	results, err := RunBaseline(context.Background(), Deps{CH: srv.URL, DB: "vikasa_gdot"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(results) != 3 {
...
func TestRunDedupOptimizesThenComparesCounts(t *testing.T) {
	...
	res, err := RunDedup(context.Background(), Deps{CH: srv.URL, DB: "vikasa_gdot"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !sawOptimize {
