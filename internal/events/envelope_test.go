...
func TestContentAddressedID(t *testing.T) {
	...
	a, err := New("gdot", "d1", "cab-i85-001", "asc-1", "vikasa.signal-control.phase-state-change.v1", at, msg)
	if err != nil {
		t.Fatal(err)
	}
	b, _ := New("gdot", "d1", "cab-i85-001", "asc-1", "vikasa.signal-control.phase-state-change.v1", at, msg)
	if a.ID != b.ID {
		t.Fatal("same content must produce same ID")
	}
	msg2 := &signalcontrolv1.PhaseStateChange{SourceDeviceId: "asc-1", PhaseNumber: 4}
	c, _ := New("gdot", "d1", "cab-i85-001", "asc-1", "vikasa.signal-control.phase-state-change.v1", at, msg2)
	if a.ID == c.ID {
...
func TestHeadersCarryCEAndMsgID(t *testing.T) {
	at := time.Now().UTC()
	e, _ := New("gdot", "d1", "cab-002", "dms-1", "vikasa.dms.mode-changed.v1", at, &signalcontrolv1.PhaseStateChange{})
	h := e.Headers()
...
func TestFromMsgRoundTrip(t *testing.T) {
	at := time.Date(2026, 7, 14, 12, 0, 0, 0, time.UTC)
	msg := &signalcontrolv1.PhaseStateChange{SourceDeviceId: "asc-1", PhaseNumber: 2}
	env, err := New("gdot", "d1", "cab-i85-001", "asc-1", "vikasa.signal-control.phase-state-change.v1", at, msg)
	if err != nil {
		t.Fatal(err)
	}
	natsMsg := &nats.Msg{
