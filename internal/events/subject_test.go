...
func TestSubject(t *testing.T) {
	s, err := Subject("mardot", "d1", "cab-i85-001", "signal-control", "asc-1", "phase-state-change")
	if err != nil {
		t.Fatal(err)
	}
	want := "vikasa.mardot.d1.cab-i85-001.signal-control.asc-1.phase-state-change"
	if s != want {
...
func TestSubjectRejectsBadTokens(t *testing.T) {
	for _, bad := range []string{"cab_001", "cab.001", "CAB-001", ""} {
		if _, err := Subject("mardot", "d1", bad, "dms", "dms-1", "mode-changed"); err == nil {
...
func TestParseSubjectRoundTrip(t *testing.T) {
	p, err := ParseSubject("vikasa.mardot.d1.cab-i85-001.signal-control.asc-1.phase-state-change")
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseShareSubject(t *testing.T) {
	p, err := ParseShareSubject("vikasa.mardot.share.i85.cab-i85-001.signal-control.asc-1.phase-state-change")
	if err != nil {
		t.Fatal(err)
	}
	want := SubjectParts{
...
func TestParseShareSubjectRejectsBadForm(t *testing.T) {
	for _, bad := range []string{
		"vikasa.mardot.d1.cab-i85-001.signal-control.asc-1.phase-state-change", // 7-token internal form
		"vikasa.mardot.share.i85.cab-i85-001.signal-control.asc-1",             // too short
		"nope.mardot.share.i85.cab-i85-001.signal-control.asc-1.phase-state-change",
	} {
