...
func TestNetworkConnectArgsConnectIncludesAlias(t *testing.T) {
	got := networkConnectArgs("connect", "vikasa-demo_vikasa", "vikasa-demo-mardot-cab-i85-001-nats-1", "mardot-cab-i85-001-nats")
	want := []string{"network", "connect", "vikasa-demo_vikasa", "--alias", "mardot-cab-i85-001-nats", "vikasa-demo-mardot-cab-i85-001-nats-1"}
...
func TestNetworkConnectArgsDisconnectHasNoAlias(t *testing.T) {
	got := networkConnectArgs("disconnect", "vikasa-demo_vikasa", "vikasa-demo-mardot-cab-i85-001-nats-1", "mardot-cab-i85-001-nats")
	want := []string{"network", "disconnect", "vikasa-demo_vikasa", "vikasa-demo-mardot-cab-i85-001-nats-1"}
	if !reflect.DeepEqual(got, want) {
		t.Errorf("got %v, want %v", got, want)
	}
	for _, a := range got {
