...
func runVerifyBaseline(args []string) {
	...
	dot := fs.String("dot", "", "DOT short name, e.g. mardot (required)")
...
func runTour(args []string) {
	...
	dot := fs.String("dot", "mardot", "primary DOT for the cabinet-scoped phases (wan-cut/restore/fault/corridor); baseline always checks all 3 DOTs")
	ch := fs.String("ch", "http://localhost:8123", "ClickHouse HTTP base URL")
